  ...
  const handleLogin = async (e: { preventDefault: () => void }) => {
    ...
    if (Object.values(newErrors).every((error) => !error)) {
      ...
      try {
        ...
        console.log(res?.data?.data?.user?.attributes);
        if (res?.status == 200) {
          if (
            res?.data?.data?.user?.attributes?.role === "ORGANIZATION OWNER"
          ) {
            if (res?.data?.data?.user?.attributes?.fully_onboarded === true) {
              push("/account");
            } else {
              push("/onboarding");
            }
            ...
            if (res?.data?.data?.user?.attributes?.team_member_onboarded) {
              if (res?.data?.data?.user?.attributes?.fully_onboarded === true) {
                push("/account");
              } else {
    ...
    <section className=" flex items-center justify-center h-screen ">
      <div className="form-container lg:w-3/12 w-10/12 mx-auto">
        ...
        <form onSubmit={handleLogin}>
          ...
          <Button className=" lg:mt-8 mt-6 font-semibold bg-[--primary] hover:bg-blue-500 w-full h-[53px] text-white">
            {loading ? "Loading..." : "Log In"}
          </Button>
          <p className="text-center lg:mt-8 mt-6">
